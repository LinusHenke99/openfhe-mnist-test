...
const std::string MODEL_PATH = "../model/cryptonet/";
...
int main(int argc, char** argv) {
    ...
    DeserializeCiphertext(cipher, filename, cryptonet);
    auto desEnd = Clock::now();

    std::cout << "Loading model...";

    //  Loading the convolutional layer weights and biases
    auto convWeights = loadMatrix(MODEL_PATH + "_Conv_0_weights.csv");
    convWeights = transpose(convWeights);
    std::vector<double> convBiasesVec = loadBias(MODEL_PATH + "_Conv_0_bias.csv");
    Plaintext convBiases = context->MakeCKKSPackedPlaintext(convBiasesVec);

    //  Loading the gemm3 weights and biases
    auto gemm1Weights = loadMatrix(MODEL_PATH + "_Gemm_3_w.csv");
    gemm1Weights = transpose(gemm1Weights);
    std::vector<double> gemm1BiasesVec = loadBias(MODEL_PATH + "_Gemm_3_bias.csv");
    Plaintext gemm1Biases = context->MakeCKKSPackedPlaintext(gemm1BiasesVec);

    // Loading the gemm5 weights and biases
    auto gemm2Weights = loadMatrix(MODEL_PATH + "_Gemm_5_w.csv");
    gemm2Weights = transpose(gemm2Weights);
    std::vector<double> gemm2BiasesVec = loadBias(MODEL_PATH + "_Gemm_5_bias.csv");
